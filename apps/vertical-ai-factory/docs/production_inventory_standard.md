...
| 코드 | 유형 | 설명 |
|------|------|------|
| `BOH` | 기초 재고 | Beginning On Hand - 기간 시작 시점 재고 |
| `EOH` | 기말 재고 | Ending On Hand - 기간 종료 시점 재고 |
...
| 유형 | 코드 | 설명 | 처리 방법 |
|------|------|------|----------|
| 경결함 | `MINOR_DEFECT` | 경미한 불량 | Rework 후 양품화 |
| 중결함 | `MAJOR_DEFECT` | 심각한 불량 | Rework or 폐기 판정 |
| 치명결함 | `CRITICAL_DEFECT` | 치명적 불량 | 즉시 폐기 |
...
| 사유 코드 | 설명 | 해제 조건 |
|-----------|------|----------|
| `QC_PENDING` | 정기 품질 검사 대기 | 검사 완료 |
| `SPEC_CHANGE` | 스펙 변경 확인 필요 | 기술팀 확인 |
| `INVESTIGATION` | 공정 이상 조사 중 | 원인 분석 완료 |
| `CUSTOMER_REQ` | 고객 요청 보류 | 고객 승인 |
...
| 품질 판정 | 미확정 (대기) | 확정 (불량) |
| 다음 공정 | 이동 불가 | 이동 불가 |
| 처리 방향 | 검사 후 결정 | Rework or 폐기 |
| 재고 위치 | 현재 공정 유지 | Rework 창고 이동 |
