...
export default function SC011507Page() {
    // 검색 조건 상태
      ...
      <div className="min-h-screen bg-slate-50">
        ...
        <div className="flex items-center gap-4 p-4 bg-white border-b">
          <YearMonthPicker value={searchYearMonth} onChange={setSearchYearMonth} label="기준월" />
            <ProductSelect value={searchProduct} onChange={setSearchProduct} label="제품" />
            <button onClick={handleSearch} className="btn-primary"><Search className="h-4 w-4 mr-1" />조회</button>
        </div>
      </div>
    );
}
