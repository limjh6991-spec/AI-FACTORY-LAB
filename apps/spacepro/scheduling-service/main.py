...
# 라우터 등록 (리팩토링된 모듈)
from routers import routing_router, simulation_router
...
async def create_routing(request: dict):
    ...
    Request:
    {
        ...
        "revision": "1.0",
        "routing": [
            {"op_seq": 10, "op_name": "area_1", "machine_code": "EQ-1-01", "setup_time": 10, "cycle_time": 2, "process_yield": 98}
        ]
    }
    """
    try:
            ...
            with conn.cursor() as cur:
                for step in routing:
                    cur.execute("""
                        ...
                        ON CONFLICT (item_code, revision, op_seq) 
                        DO UPDATE SET op_name = EXCLUDED.op_name,
                                     machine_code = EXCLUDED.machine_code,
                                     setup_time = EXCLUDED.setup_time,
                                     cycle_time = EXCLUDED.cycle_time,
                                     process_yield = EXCLUDED.process_yield
                    """, (
                        item_code, revision, step.get('op_seq'),
                        step.get('op_name'), f"WC-AREA-{step.get('op_name', '').split('_')[-1]}",
                        step.get('machine_code'),
                        step.get('setup_time', 10), step.get('cycle_time', 1),
                        step.get('process_yield', 95)
                    ))
                conn.commit()
        
        return {"success": True, "item_code": item_code, "steps_created": len(routing)}
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def update_routing(item_code: str, request: dict):
    ...
    try:
        ...
        return {"success": True, "item_code": item_code, "steps_updated": len(routing)}
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
@app.delete("/routing/{item_code}")
async def delete_routing(item_code: str, revision: str = "1.0"):
    """제품 라우팅 삭제"""
    try:
        with get_db_connection() as conn:
            with conn.cursor() as cur:
